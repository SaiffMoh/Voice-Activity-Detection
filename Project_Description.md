...
Suggested visual (Mermaid) diagram you can paste into Markdown viewers that support it:

```mermaid
sequenceDiagram
  participant Browser
  participant API
  participant ElevenLabs as ElevenLabs(STT/TTS)
  participant Watson as Watson(LLM)

  Browser->>Browser: Silero VAD detects voice
  Browser->>API: POST /api/transcribe (audio wav + messageHistory)
  API->>ElevenLabs: STT (speech-to-text)
  ElevenLabs-->>API: transcript
  API->>Watson: stream messages (LLM)
  Watson-->>API: streamed tokens
  API->>ElevenLabs: TTS (text->pcm/wav)
  ElevenLabs-->>API: audio/wav
  API-->>Browser: audio/wav + X-Transcript + X-Response headers
